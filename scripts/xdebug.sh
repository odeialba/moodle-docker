...
SERVICE_ID=$(bin/moodle-docker-compose ps -q webserver)

if [ "$1" == "enable" ]; then
    docker exec $SERVICE_ID bash -c \
...
docker exec $SERVICE_ID bash -c '$(php -m | grep -q Xdebug) && echo "Status: Xdebug ENABLED" || echo "Status: Xdebug DISABLED"'
